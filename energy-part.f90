!     energy partitioning for A(multi-atom-system), B(atom): subha
...
!         INDB(I) = 0
!      ENDDO
!---------------- change by malik sep2019 ----------------
!         FOR AMB5 WITH 104 ATOMS AND Ar PROJECTILE
...
!        WRITE(6,*) "ECMTA, ECMTB, VCMB", ECMTA, ECMTB,VCMB
!     Calculate the relative translational energy of the system
!     The center of mass information is obtained from the subroutine
...
!
      ...
      DO I=1, 3
         ...
         VR(I) = VCMA(I) - VCMB(I)
         RCM = RCM + QR(I)**2
!         WRITE(6,*) "VCMA, VCMB, VR(I)", VCMA(I), VCMB(I), VR(I)
      ENDDO
      RCM = SQRT(RCM)
      WRITE(6,*) "COM_DIS", RCM
      VREL = 0.0D0
...
!      write(6,*) vrelsq, rdmass, rcm
      ...
      DO I=1, N
         ...
         J1 = J2 - 1
         IF (INDA(I) .GE. 0.5) THEN
            PSQ = PA(J1)**2 + PA(J2)**2 + PA(J3)**2
            CMRVA(I) = PSQ/2.0D0/C1/W(I)
!         WRITE(6,*)"PA",PA(J1),PA(J2),PA(J3)
         ELSE
            CMRVA(I) = 0
         ENDIF
!         WRITE(6,*)"ECMRVA, CMRVA", ECMRVA,CMRVA(I)
...
!                Write(6,*) "check"
      ...
      ENDDO
!      WRITE(6,*)"ECMRVA, ECMRVB", ECMRVA, ECMRVB
      EVIBA = ECMRVA - EROTA
      EVIBB = ECMRVB - EROTB
...
!
      SUBROUTINE CENMAS(DOF, SW, QCM, QCMB, VCM, NA, P, Q, PP, QQ, W)
      INTEGER :: DOF
      REAL :: PP(DOF), QQ(DOF), P(DOF), Q(DOF)
      REAL :: QCM(3), VCM(3), W(NA), CM_DIS, QCMB(3)
